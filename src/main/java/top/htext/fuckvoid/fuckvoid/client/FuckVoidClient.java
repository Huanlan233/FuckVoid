    ...
    public void onInitializeClient() {
        ...
        ClientTickEvents.END_CLIENT_TICK.register((MinecraftClient client) -> {
            ...
            if (player != null && !player.isCreative() && !player.isSpectator() && player.getY() < 0) {
                client.inGameHud.setTitleTicks(0, 1, 0);
                client.inGameHud.setTitle(new TranslatableText("title.fuckvoid.warning").formatted(Formatting.RED));
                player.playSound(new SoundEvent(new Identifier(Identifier.DEFAULT_NAMESPACE, "entity.arrow.hit_player")), SoundCategory.AMBIENT, 1.0f, 1.0f);
            }
        });
    }
}
